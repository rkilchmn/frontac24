...
function print_GL_transactions()
{
	...
	while ($account=db_fetch($accounts))
	{
		...
		$rep->TextCol(4, 6,	_("Ending Balance"));
			if ($total > 0.0)
			$rep->TextCol(8, 9, number_format2( abs($total), $dec)); // total is float and needs to be converted to decimal
			// $rep->AmountCol(8, 9, abs($total), $dec);
		else
			$rep->TextCol(9, 10, number_format2( abs($total), $dec)); // total is float and needs to be converted to decimal
